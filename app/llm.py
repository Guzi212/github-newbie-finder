...
SYSTEM_PROMPTS: dict[str, str] = {
    ...
    "rerank_repos": (
        ...
        "- LANGUAGE: the input has a top-level `preferred_language` field ('zh' or 'en'). "
        "ALL natural-language fields you produce — `summary`, `pros`, `cons`, `differentiators`, "
        "`risk_flags`, `missing_info` — MUST be in that language. "
        "'zh' = 简体中文 (use natural conversational Chinese, not literal translation); "
        "'en' = English. Do NOT mix languages in a single field. "
        "EXCEPTIONS that stay verbatim regardless of language: `full_name`, `deployment_level`, "
        "`evidence[*].excerpt` (keep the source language of the README snippet), and "
        "`evidence[*].source_url`. Code identifiers, library names, and CLI flags also stay verbatim.\n"
        "- `summary` MUST be a clear 1-2 sentence description of WHAT THE PROJECT DOES (the elevator pitch). "
        "When `preferred_language='zh'`, write it in Chinese even if the README is English — translate the "
        "essence, do not just copy.\n"
    ...
    "generate_tutorial": (
        ...
        "- LANGUAGE: read `user_profile.preferred_language` ('zh' or 'en'). ALL natural-language fields "
        "you produce — `assumptions`, `prerequisites`, `steps[*].title`, `steps[*].explanation`, "
        "`verification`, `common_errors[*].symptom/cause/fix`, `rollback`, `next_steps` — MUST be in that "
        "language. 'zh' = 简体中文 (natural conversational tone, not literal); 'en' = English. "
        "EXCEPTIONS: shell commands inside `commands[]` stay raw (never translate command text, paths, "
        "library names, or CLI flags). Inside `explanation`, code identifiers can be kept in English.\n"
